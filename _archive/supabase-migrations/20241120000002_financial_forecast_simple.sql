-- Financial Forecast Tables - Simplified Version
-- Run this version if the other one has issues

-- Drop existing tables if they exist (for clean slate)
DROP TABLE IF EXISTS forecast_payroll_summary CASCADE;
DROP TABLE IF EXISTS forecast_employees CASCADE;
DROP TABLE IF EXISTS forecast_pl_lines CASCADE;
...
CREATE TABLE financial_forecasts (
  ...
  year_type TEXT NOT NULL CHECK (year_type IN ('CY', 'FY')),
...
CREATE TABLE forecast_employees (
  ...
  weekly_payg DECIMAL(10, 2),
  super_rate DECIMAL(5, 2) DEFAULT 11.0,
  sort_order INTEGER DEFAULT 0,
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
...
-- ============================================================================
CREATE TABLE forecast_payroll_summary (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  forecast_id UUID NOT NULL,
  pay_runs_per_month JSONB DEFAULT '{}',
...
ALTER TABLE financial_forecasts
  ADD CONSTRAINT fk_forecast_xero_connection
  FOREIGN KEY (xero_connection_id)
  REFERENCES xero_connections(id)
  ON DELETE SET NULL;

ALTER TABLE forecast_pl_lines
  ADD CONSTRAINT fk_pl_lines_forecast
  FOREIGN KEY (forecast_id)
  REFERENCES financial_forecasts(id)
  ON DELETE CASCADE;

ALTER TABLE forecast_employees
  ADD CONSTRAINT fk_employees_forecast
  FOREIGN KEY (forecast_id)
  REFERENCES financial_forecasts(id)
  ON DELETE CASCADE;

ALTER TABLE forecast_payroll_summary
  ADD CONSTRAINT fk_payroll_forecast
  FOREIGN KEY (forecast_id)
  REFERENCES financial_forecasts(id)
  ON DELETE CASCADE;
...
ALTER TABLE xero_connections
  ADD CONSTRAINT unique_business_tenant
  UNIQUE(business_id, tenant_id);

ALTER TABLE forecast_pl_lines
  ADD CONSTRAINT unique_forecast_account
  UNIQUE(forecast_id, account_code);

ALTER TABLE forecast_payroll_summary
  ADD CONSTRAINT unique_forecast_payroll
  UNIQUE(forecast_id);
...
-- Xero Connections
CREATE POLICY "Users can manage their xero connections"
...
-- Forecast P&L Lines
CREATE POLICY "Users can manage pl lines"
  ON forecast_pl_lines FOR ALL
  USING (
    EXISTS (
...
CREATE POLICY "Users can manage payroll summary"
  ON forecast_payroll_summary FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM financial_forecasts
