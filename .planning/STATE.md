...
Phase: 66 (section-permission-followups) — **IN PROGRESS** (3/4 plans complete 2026-05-17; 66-01 at human-verify checkpoint). 66-02 normalized the 3 consolidated routes (`consolidated`, `consolidated-bs`, `consolidated-cashflow`) to resolve dual IDs via `resolveBusinessIds` + ids.bizId everywhere + regression test. 66-03 produced `66-SERVICE-ROLE-AUDIT.md` (per-route disposition of 32 finance routes: 11 auth-bound, 11 keep, 10 convert, 0 carve). 66-04 produced `66-OPS-ADMIN-SECTION-PERMISSION-AUDIT.md` — all 16 ops/admin/coach routes need no gate. **66-01 (critical path) tasks 1-2 done:** audit script `scripts/audit-section-permissions-legacy-key.ts` + idempotent backfill migration `20260516000000_phase66_backfill_finances_section_key.sql` (covers business_users + team_invites). **66-01 Task 3 PENDING operator:** run the audit against prod, verify the migration idempotent on a Supabase preview branch, apply to prod. This migration MUST land in production before Phase 65 Wave 65-04 flips `SECTION_PERMISSION_ENFORCE=true`.
Plan: 66-01 awaiting human-verify; 66-02/03/04 complete
...
Last activity: 2026-05-17 -- Phase 66 Wave 1 executed; 66-02/03/04 complete, 66-01 at human-verify checkpoint
