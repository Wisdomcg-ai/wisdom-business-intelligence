...
- [x] 44-08-PLAN.md — [Sub-phase C] ForecastReadService + D-18 runtime invariants
...
- [x] 44-02-PLAN.md — Remove `eslint.ignoreDuringBuilds` from `next.config.js` and fix any lint fallout
- [x] 44-03-PLAN.md — Expand `supabase-preview.yml` into 5 parallel jobs: migration-check + lint + typecheck + vitest + build
- [x] 44-04-PLAN.md — Create `playwright-nightly.yml` (cron 14:00 UTC, manual dispatch, HTML report on failure)
- [ ] 44-05-PLAN.md — Branch protection + secret provisioning + smoke-test the gates (human checkpoints)
...
|-------|----------------|--------|-----------|
| 44. Test Gate & CI Hardening | 4/5 | In Progress|  |
...
- [x] 69-03-PLAN.md — Code fix mapped 1:1 to 69-DIAGNOSIS root cause(s) + regression tests
- [x] 69-04-PLAN.md — Pre-expiry Sentry warning (24h lead time) + XeroHealthPill verification + monitoring runbook
...
- [x] 70-02-PLAN.md — A1: Active-forecast remediation across all production businesses (D1)
...
- [~] 70-07-PLAN.md — B3: IICT 5-step onboarding completion (profile + consolidation mode + subs + FY27 dedupe + baseline snapshots; interactive) — **DEFERRED 2026-05-31** per Matt (same pattern as 70-06; Steps 1+3 need IICT business data not at hand — industry, revenue, profit, canonical subs list — deferred to focused IICT coach session). Script committed 3cb30e71 (727 LOC) but uninvoked; preserved with three build-time deviations baked in (D1 `consolidation_budget_mode='per_tenant'` not 'consolidated' per CHECK constraint; D2 Step 5 UI-driven; D3 Step 4 already no-op). See `.planning/phases/70-.../70-07-SUMMARY.md`.
...
Plans:
- [ ] 71-01-PLAN.md — B2: vendor-key normalization consolidation (commentary route keying alignment + round-trip + single-source-of-truth test)
- [ ] 71-02-PLAN.md — B1: wages employee fuzzy name match (tokenSort + Levenshtein fallback + Sentry telemetry on fuzzy hits)
- [ ] 71-03-PLAN.md — B3: Proceed-as-Draft persistence (immediate snapshot save + toast + integration test)
- [ ] 71-04-PLAN.md — S1: commentary trigger expansion (revenue shortfalls + favourable expense + BS movements + trigger_reason)
- [ ] 71-05-PLAN.md — S2: subscription budget-only vendor visibility (zero-actual rows + "not billed this month" badge)
- [ ] 71-06-PLAN.md — S3: wages per-payrun expand UI (chevron + per-payrun detail row)
- [ ] 71-07-PLAN.md — S4: PDF variance polarity refactor (decideTintColor helper + _polarity cell metadata)
- [ ] 71-08-PLAN.md — S5: Balance Sheet equation check + red banner when |residual| > $1
- [ ] 71-09-PLAN.md — S6: multi-tenant non-AUD redirect toast (one-time per session per business via localStorage)
- [ ] 71-10-PLAN.md — D4: snapshot serializer (numeric→named section keys) + remap migration script with dry-run/--apply checkpoint
...
### Phase 61: Selective List Sharing (Todos & Ideas)
