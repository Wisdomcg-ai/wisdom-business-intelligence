...
// VALID-04 (observe mode): POST derives bank balances by month from Xero.
const BankBalancesPostSchema = z
  .object({
...
async function postHandler(request: Request) {
  try {
    ...
    const _sectionBlocked = enforceSectionPermission(
      _sectionVerdict,
      'finances',
      'api/forecast/cashflow/bank-balances',
      user.id,
      business_id,
    )
...
export const POST = withSchema('forecast/cashflow/bank-balances', BankBalancesPostSchema, postHandler)
