...
import { loadAccountActuals } from '@/lib/monthly-report/account-actuals-load'
 ...
 * figures; the page prints the sentence.
 */
...
async function getHandler(request: Request) {
  try {
    ...
    if (!businessId || !isValidPeriodMonth(endMonth) || !(months >= 1 && months <= 24)) {
    ...
    const result = await loadAccountActuals(supabase, businessId, endMonth, months, codes)
