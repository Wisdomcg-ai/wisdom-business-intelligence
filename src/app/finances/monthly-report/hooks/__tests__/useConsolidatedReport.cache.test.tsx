...
import { useConsolidatedReport, cachedConsolidatedFor } from '../useConsolidatedReport'
...
describe('the consolidated cache knows which month it holds', () => {
  it('a July report is not the report for an August export', async () => {
    ...
    expect(api.current!.reportFor).toEqual({ reportMonth: '2026-07', fiscalYear: 2027 })
    expect(cachedConsolidatedFor(api.current!, '2026-08', 2027)).toBeNull()
    expect(cachedConsolidatedFor(api.current!, '2026-07', 2027)).toMatchObject({ label: '2026-07' })
    expect(cachedConsolidatedFor(api.current!, '2026-07', 2026)).toBeNull()
  })
  ...
  it('Generate’s own consolidated report replaces the cache, and a load that started before it cannot put the old one back', async () => {
    ...
    expect(cachedConsolidatedFor(api.current!, '2026-08', 2027)).toBe(fresh)
  ...
  it('a slower July load landing after an August one does not take August’s place', async () => {
    ...
    expect(api.current!.reportFor).toEqual({ reportMonth: '2026-08', fiscalYear: 2027 })
    expect(api.current!.isLoading).toBe(false)
  })
  ...
  it('the page wires it: the export reads the cache by month, Generate primes it, both pre-flights get the currencies', () => {
    ...
    expect(pdfLoad).toContain('cachedConsolidatedFor({ report: consolidatedReport, reportFor: consolidatedReportFor }')
  ...
  it('clear forgets the month too', async () => {
    ...
    expect(api.current!.reportFor).toBeNull()
    expect(cachedConsolidatedFor(api.current!, '2026-08', 2027)).toBeNull()
  })
})
