    ...
    quickActions: [
      {
        label: 'Summary',
        icon: '📋',
        promptBuilder: (state: ForecastWizardState, fiscalYear: number, activeYear: 1 | 2 | 3) => {
          return `Please provide a brief summary of my FY${(fiscalYear + activeYear - 1).toString().slice(-2)} forecast. What are the key takeaways?`;
        },
      },
    ],
    quickQuestions: [
      'Is my forecast complete?',
      'What should I double-check?',
      'Any final recommendations?',
    ],
    contextBuilder: (state: ForecastWizardState, fiscalYear: number, activeYear: 1 | 2 | 3) => {
      return `
**Step 9: Final Review**
Reviewing completed forecast for FY${fiscalYear + activeYear - 1}.
`;
    },
  },
};
