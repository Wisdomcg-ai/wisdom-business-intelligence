...
export default function CoachMessagesCard({
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border-l-4 border-l-brand-orange border border-gray-200 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-gray-100 bg-brand-orange/5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-brand-orange/10 rounded-lg flex items-center justify-center relative">
              <MessageCircle className="h-4 w-4 text-brand-orange" />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center px-1 text-xs font-bold text-white bg-brand-orange rounded-full">
                  {unreadCount > 9 ? '9+' : unreadCount}
                </span>
              )}
            </div>
            <div>
              <h3 className="font-semibold text-brand-navy">Coach Messages</h3>
