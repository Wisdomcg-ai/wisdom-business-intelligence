...
describe('R30 — getLastSyncByTenant', () => {
  ...
  describe('tenantIds — a one-business caller reads only its own orgs', () => {
        ...
        from: () => {
          ...
          const chain: any = {
            ...
            then: (resolve: any, reject: any) =>
              Promise.resolve({ data: [{ tenant_id: 't1', finished_at: isoAgo(1000) }], error: null }).then(resolve, reject),
          }
          return chain
        },
      }
      return { client, inCalls, reads: () => reads }
    }

    it('narrows the read to the given tenants', async () => {
      const { client, inCalls } = recordingClient()
      const { ok, byTenant } = await getLastSyncByTenant(client as any, 60, ['t1', 't2'])
      expect(ok).toBe(true)
      expect(byTenant.has('t1')).toBe(true)
      expect(inCalls).toContainEqual(['tenant_id', ['t1', 't2']])
    })

    it('an empty list asks nothing — no org, nothing to look up — and is ok, not a failure', async () => {
      const { client, reads } = recordingClient()
      const result = await getLastSyncByTenant(client as any, 60, [])
      expect(result.ok).toBe(true)
      expect(result.byTenant.size).toBe(0)
      expect(reads()).toBe(0)
    })

    it('without a list the read stays fleet-wide, as the board and the daily report need', async () => {
      const { client, inCalls } = recordingClient()
      await getLastSyncByTenant(client as any, 60)
      expect(inCalls.map(([column]) => column)).not.toContain('tenant_id')
    })
  })
})
