...
import { deriveActualCashMonth, type ActualCashReconciliation, type CashLineLabel, type CashPlRow, type MonthPayslips } from './pack-cash-actuals'
...
export function openingGst(
  ...
  reportMonth: string,
  cfg: CashModelConfig,
): { amount: number; dueMonth: string; settledEnd: string | null; leftOver: number } | { reason: string } {
  ...
  if (cfg.gst.opening === 'balance') return { amount: closing, dueMonth, settledEnd: null, leftOver: 0 }
  ...
  return { amount: round2(closing - settled), dueMonth, settledEnd, leftOver: settled }
}
...
function paymentMonthsText(name: string, from: string): string {
  if (name === 'monthly' || name === 'payday') return name === 'monthly' ? 'monthly' : 'each pay run'
...
export function buildPackCashModel(args: {
  ...
  let terms: { dso: number; dpo: number; derived: boolean } | null = null
  ...
  if (firstForecast) {
    ...
    terms = { dso, dpo, derived: needDerived }
    ...
    if (gst.settledEnd && Math.abs(gst.leftOver) >= 1) {
      warnings.push(`The GST accounts held ${fmtDollars(gst.leftOver)} at ${fmtDate(gst.settledEnd)}, a period its BAS should have settled; the model pays only the quarter-to-date movement (${fmtDollars(gst.amount)}), so that balance is in no forecast payment`)
    }
...
export function packCashModelBasis(a: {
  ...
  terms: { dso: number; dpo: number; derived: boolean } | null
  opexOnDpo: boolean
  basText: string | null
  ...
  if (a.budgetMonths.length > 0) {
    ...
    if (a.terms) {
      parts.push(`debtors ${a.terms.dso} days, creditors ${a.terms.dpo} days${a.terms.derived ? ' (from the ledger)' : ''}${a.opexOnDpo ? ', expenses on creditor days' : ''}`)
    }
